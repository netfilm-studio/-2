...
let elapsed = 0;
const TOTAL_TIME = FACTS.length * 5500; // общее время = кол-во фактов × время на факт
...
function resetAndAnimateCompat(container) {
  ...
  // Сброс счётчиков к их истинным (зафиксированным при загрузке) значениям
...
function initCounters() {
  // Зафиксировать истинные (статические) значения ДО первой анимации,
  // чтобы повторные открытия модалки не портили проценты
  document.querySelectorAll('.compat-score, .compat-row-val').forEach(el => {
    if (!el.dataset.originalValue) el.dataset.originalValue = el.textContent;
  });
...
function initAll() {
  ...
  buildEps();
  buildMovies();
  // Set welcome background — картинка или видео, определяется по расширению файла
  ...
  if (typeof BG !== 'undefined') {
    ...
    if (isVideo && wbgVideo) {
      wbgVideo.src = BG;
      wbgVideo.style.display = 'block';
      if (wbg) wbg.style.display = 'none';
      wbgVideo.play().catch(() => {}); // на некоторых мобильных нужен явный play()
    } else if (wbg) {
      wbg.style.backgroundImage = "url('" + BG + "')";
    }
  }
  // Init credits photos (may be after script)
  const p1 = document.getElementById('cr-p1');
  const p2 = document.getElementById('cr-p2');
  const p3 = document.getElementById('cr-p3');
  if (p1) p1.src = EP1;
  if (p2) p2.src = EP2;
  if (p3) p3.src = EP3;
